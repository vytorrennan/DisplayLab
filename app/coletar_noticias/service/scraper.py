...
def raspar_adrenaline():
    ...
    artigos = soup.find_all('article', class_='feed')

    for artigo in artigos[:10]: 
        try:
            # titulo e link
            titulo_tag = artigo.find('h2', class_='feed-title')
            link_tag = artigo.select_one('a', class_='feed-link')
            ...
            # descrição
            hat_tag = artigo.find('p', class_='feed-hat')
            hat_text = hat_tag.get_text(strip=True) if hat_tag else ''
            descricao = f"[{hat_text}] {titulo}" if hat_text else titulo
            ...
            # data
            data_span = artigo.find('span', class_='feed-created-data')
            data_string = data_span.get_text(strip=True) if data_span else None

            data_publicacao = datetime.now()

            if data_string:
                # Ex.: "20/11/2025 20:45"
                data_limpa = re.sub(r'[^0-9/:\s]', '', data_string).strip()

                try:
                    data_publicacao = datetime.strptime(data_limpa, "%d/%m/%Y %H:%M")
                except ValueError:
                    try:
                        data_publicacao = parse(data_limpa, dayfirst=True, fuzzy=True)
                    except ParserError:
                        print(f"Aviso: Não foi possível analisar a data: {data_limpa}")

            # fonte
            fonte_escolhida = "ADRENALINE"
